    ...
    def single_pass(self, x, edge_index, edge_attr, name=None, seq=None, batch=None):
        ...
        if self.node_emb:
            if batch is not None:
                # assert len(name) == len(seq)
                # assert len(name) == (batch[-1] + 1)
                # node_num = [0 for i in range(batch[-1] + 1)]
                # for i in batch:
                #     node_num[i] += 1
                t2 = time.time()
                acc_times[0] += (t2 - t1)
                ...
                for idx, i in enumerate(name):
                    ...
                    # special cases
                    # assert len(emb) <= node_num[idx]
                    # if len(emb) < node_num[idx]:
                    #     padding = torch.zeros((node_num[idx] - len(emb), emb.shape[-1]))
                ...
                embedding = torch.tensor(emb_file[name][()])
...
class GATModel2(nn.Module):
    def __init__(self, 
        ...
        # if embedder == 'prott5':
        #     self.embedder = ProtTransEmbedder(device=device)
        # elif embedder == 'esm1b':
